...
RIAL; DIAL; value-based; on-policy; model-free; discrete communication channel; discrete action space; continuous state space; cooperative task;



## 创新点及贡献

1、首个通过深度学习学习多智能体之间的交流协议的方法，基于 DRQN 提出了一种集中训练分散执行的多智能体学习交流协议的框架。

2、集中训练分散执行的框架在于训练时所有的智能体参数共享同一个网络，在执行时每个智能体复制一份网络并拥有自己独立的隐藏状态，其中根据交流信息是否可以反向传播拥有反馈机制分为 RIAL 方法和 DIAL 方法。
...
3、Q-Net 网络的 Q-value 输出给 Action Select 通过 $\epsilon$-贪心来选择动作，注意这里的交流动作和环境动作都是 one-hot 编码。
...
3、集中训练分散执行的框架同样通过参数共享来实现，此外在训练时 $DRU(m^a_t) = Logistic({\mathcal{N}}(m^a_t,\sigma))$，执行时 $DRU(m^a_t) = {\mathbb{1}}_{m^a_t > 0}$，注意这里的交流动作是二进制编码，而 RIAL 的交流动作是 one-hot 编码。
