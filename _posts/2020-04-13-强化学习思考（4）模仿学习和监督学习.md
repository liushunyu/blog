...
1、train $\pi_\theta(a_t|o_t)$ from human data $\cal{D} = \{o_1, a_1,...,o_N, a_N\}$

2、run $\pi_\theta(a_t|o_t)$ to get dataset $\cal{D}_\pi = \{ o_1, ..., o_M \}$

3、Ask human to label $\cal{D}_\pi$ with actions $a_t$

3、Aggregate: $\cal{D} \leftarrow \cal{D} \cup \cal{D}_\pi$
