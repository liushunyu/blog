...
1. agent 和环境进行互动中得到的 reward 十分稀疏。
2. 在某些任务中很难定义 reward。
3. 人为设计的 reward 可能会得到不受控制的 policy。
...
在 IRL 中，没有固定的 reward function，而是用一个专家来和环境做互动并学到一个 reward function，然后这个 reward function 才会被用来训练 actor。
