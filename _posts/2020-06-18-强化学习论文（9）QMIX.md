...
QMIX; value-based; off-policy; model-free; discrete action space; continuous state space; cooperative task; credit assignment; centralized training with decentralized execution; 
...
1、VDN 使用对单智能体的价值函数直接求和的方法计算中心价值函数，但是限制住了团队价值函数的复杂性表达，而且没有利用到全局的状态信息。
...
<img width="70%" src="/img/in-post/2020-06-18-强化学习论文（9）QMIX.assets/image-20200618111341687.png"/>



2、QMIX 通过提出单调性假设放松了 VDN 中对单智能体的价值函数直接求和等于团队价值函数的约束限制，其中 $Q_a$ 为单智能体的价值函数，$Q_{tot}$ 为团队价值函数，$a\in A$ 是智能体。

<img width="50%" src="/img/in-post/2020-06-18-强化学习论文（9）QMIX.assets/image-20200618105713386.png"/>



3、其主要结构与 VDN 类似，重点修改在于引入将额外状态信息加入到单智能体的价值函数到团队价值函数的映射过程，并将其称为 mixing network。
