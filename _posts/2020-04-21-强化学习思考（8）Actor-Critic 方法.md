...
1、sample $\mathbf{s}_{i}, \mathbf{a}_{i} \text{   from   } $\pi_{\theta}(\mathbf{a} \mid \mathbf{s})$ (run it on the robot)
...
2、update $\hat{V}_{\phi}^{\pi} \text{  using target  }  r+\gamma \hat{V}_{\phi}^{\pi}$
