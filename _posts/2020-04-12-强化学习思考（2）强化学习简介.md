...
- if $\pi_\theta(a|s)=\alpha$, $\Bbb{E}[r(s,a)] = \alpha$ is smooth
...
一般强化学习算法遵循这三步：generate samples、fit a model / estimate the return、improve the policy
...
- Value-based: if we have policy $\pi$, and we know Q"(s, a), then we can improve $\pi$
  - set $\pi'(a|s)=1$ if $a=\arg \max_a Q^\pi(s,a)$
