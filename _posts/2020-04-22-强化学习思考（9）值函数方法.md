...
- warning:
  ...
  - $V^*$: value function for optimal policy $\pi^*$, this is what value iteration does
...
3、we can prove that value iteration reaches $V^*$ because operator ${\cal{}B}$ is a contraction, if we choose $V^*$ as $\bar{V}$
